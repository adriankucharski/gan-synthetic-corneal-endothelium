...
if __name__ == '__main__':
    ...
    num_of_data_text = st.text_input(
        'Number of data to generate', value=5, type='default')
    ...
    with st.expander('Mosaic generator parameters', expanded=False):
        ...
        model_gaussian_dropout_enabled = st.checkbox('Model Gaussian Dropout', True)
    ...
    if generate_button:
        synthetic_masks, synthetic_images = generate_dataset_from_generators(
            [generator], hexagon_generator_params, model_gaussian_dropout_enabled)
        ...
        if show_data:
            ...
            for i in range(min(num_of_data, max_num_to_show)):
                images.append(
                    np.hstack([synthetic_masks[i], synthetic_images[i]]))
